...
app.get("/posts/:postTile", function (req, res) {
  const requestedTitle = lodash.lowerCase(req.params.postTile);
  let check = false;
  posts.forEach(function (post) {
    const postName = lodash.lowerCase(post.postTitle);
    if (postName === requestedTitle) {
...
app.post("/compose", function (req, res) {
  ...
  const userPost = req.body.postText;
  const post = {
    postTitle: title,
    postBody: userPost
  };
